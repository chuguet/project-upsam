...
public class CopyOfTranscodeBackground extends AsyncTask<Void, Integer, Integer>
{
	...
	public CopyOfTranscodeBackground(BaseWizard act, IFfmpgefRemoteServiceBridge remote, int notificationId) {
		_act = act;
		_remote = remote;
	...
	private int calcProgress() {
		...
		if (Prefs.durationOfCurrent != null) {
			...
			else if (currentTimeStr.equals("maybe_error") && _prevProgress == 0) {
			...
			Log.w(Prefs.TAG, "Prefs.durationOfCurrent is null, force setting: " + Prefs.durationOfCurrent);
		}
		return progress;
	}
