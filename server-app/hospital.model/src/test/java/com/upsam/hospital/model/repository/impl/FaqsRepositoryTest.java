...
public class FaqsRepositoryTest extends UnitTest {
	...
	public void canFindAFaq() throws SQLException, DataBaseException {
		...
		assertThat("pregunta", is(equalTo(expectedFaq.getPregunta())));
		assertThat("respuesta", is(equalTo(expectedFaq.getRespuesta())));
	}

	// @Test
	// public void canFindBySeccion() throws SQLException, DataBaseException {
	// List<Faq> faqs = CreateFaqsListWithSize(5);
	// when(hibernateTemplate.find("Select new Faq(id, seccion, fotos, titulo, descripcion) from Faqs where (seccion = ?)",
	// ParteDelCuerpo.HOMBRO.name()))
	// .thenReturn(faqs);
	//
	// List<Faq> expectedFaqs =
	// faqsRepository.findBySeccion(ParteDelCuerpo.HOMBRO.name());
	//
